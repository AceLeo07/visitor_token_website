    ...
    try {
      ...
      try {
        ...
        data = {
          success: res.ok,
          valid: false,
          message: raw || "Unexpected response",
        };
      }
      setVerif(data);
      if (data.valid)
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {auth.user?.name} - Gate Access Control
                </p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex gap-2 mb-6">
          ...
          <Button
            variant={tab === "scanner" ? "default" : "outline"}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card>
                ...
                <CardContent className="pt-0 text-3xl font-bold text-orange-600 flex items-center gap-2">
                  <Camera className="w-6 h-6" /> {dash?.stats.todayScanned ?? 0}
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Manual Checks</CardTitle>
                </CardHeader>
                <CardContent className="pt-0 text-3xl font-bold text-blue-600 flex items-center gap-2">
                  <List className="w-6 h-6" /> {dash?.stats.todayManual ?? 0}
                </CardContent>
              </Card>
            </div>
            <Card className="mt-8">
        ...
        {tab === "scanner" && (
          <div className="grid md:grid-cols-2 gap-8">
            <QRScanner
              ...
              onQRCodeDetected={(qr) => verifyToken(undefined, qr)}
            />
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Hash className="w-5 h-5" /> Manual Token Entry
                </CardTitle>
                <CardDescription>
                  Enter token code if QR scanning fails
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {verif && (
                  <div
                    className={`p-3 rounded-lg ${verif.valid ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
                  >
                    <p
                      className={`font-medium ${verif.valid ? "text-green-800" : "text-red-800"}`}
                    >
                      {verif.message}
                    </p>
                  </div>
